...
TEST_F(DeviceTestSuit, DeviceFailedWrite) {
  ...
  try {
    ...
    EXPECT_STREQ("Actual number of writen bytes(0) doesn't match the "
                 "expected(64): Zero buffer/length",
                 err.what());
  } catch (...) {
    FAIL()
...
TEST_F(DeviceTestSuit, DeviceReportFailedSend) {
  try {
    ...
    EXPECT_STREQ("Actual number of writen bytes(-1) doesn't match the "
                 "expected(1): Zero buffer/length",
                 err.what());
  } catch (...) {
    FAIL()
        << "Expected std::runtime_error, but a different exception was thrown";
